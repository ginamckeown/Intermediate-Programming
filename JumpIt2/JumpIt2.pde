...
void mousePressed() {
  ...
  if (homeScreenOpen) { // if on homescreen, these buttons are avalible
    ...
    if (start.pressed()) {
      ...
      // Others tabs are no longer open when game is open
      leaderBoardOpen = false;
      homeScreenOpen = false;
    } 
...
void displayLeaderBoard() {
  if (leaderBoardOpen) {
    ...
    textSize(60);
    fill(0, 40); // shadow
    text("Leader Board", width/2 - 4, height/7 + 4 ); 
    fill(196, 151, 27);
...
void displayGame() {
  if (gameOpen) {
    ...
    player.display();
    player.gameOver();
    player.jumpingIs(spacePressed);

    // display platforms
    for (Platform platform : platform) {
      platform.display();
      platform.move();
      if (platform.getX() < 0 - platform.getW()) {
        platform.restart();
      }
      player.collision(platform);
    }

    // platforms check for each other
    int platformSpacing = 100;
    platform[0].setSpeedTo(2);
    if (platform[0].getX() < width - platform[0].getW() - platformSpacing) {
      platform[1].setSpeedTo(2); // next platform moves
    }
    if (platform[1].getX() < width - platform[1].getW() - platformSpacing) {
      platform[2].setSpeedTo(2); // next platform moves
    } 
    if (platform[2].getX() < width - platform[2].getW() - platformSpacing) {
      platform[0].setSpeedTo(2); // next platform moves
    }
    
    // display inital platform
    //int initPlatX = 0;
    //int initPlatY = 300;
    //int initPlatW = int(width - platform[0].getW() - platformSpacing);
    //int initPlatH = 50;
    //int initPlatS = 2;
    
    //rect(initPlatX, initPlatY, initPlatW, initPlatH);
    //if (initPlatX > 0 - initPlatW) {
    //  initPlatX += initPlatS;
    //}
  }
}
